...
public class MiniServiceResponse {
    ...
    @Schema(description = "服务价格，单位为分", example = "0")
    ...
    @Schema(description = "时段结束时间", example = "09:30")
    private final String endTime;

    @Schema(description = "剩余可预约容量", example = "4")
    ...
    private MiniServiceResponse(ReservationPersistenceService.MiniService service) {
        ...
        this.priceCent = service.priceCent();
        this.durationMinutes = service.durationMinutes();
        this.slotId = service.slotId();
        this.serviceDate = service.serviceDate();
        this.startTime = service.startTime();
        this.endTime = service.endTime();
        this.remainingCapacity = service.remainingCapacity();
    }

    public static MiniServiceResponse from(ReservationPersistenceService.MiniService service) {
        return new MiniServiceResponse(service);
    }
