...
public class AdminReservationController {
    ...
    public ServiceResponse createService(@Valid @RequestBody CreateServiceRequest request){return ServiceResponse.from(service.createService(scope(),new ReservationPersistenceService.CreateServiceCommand(request.name(),request.paymentMode(),request.priceCent(),request.durationMinutes())));}
    @Operation(summary = "创建预约时段", description = "为指定服务项目创建带容量限制的可预约时段")
    @PostMapping("/slots") @ResponseStatus(HttpStatus.CREATED)
    public SlotResponse createSlot(@Valid @RequestBody CreateSlotRequest request){return SlotResponse.from(service.createSlot(scope(),new ReservationPersistenceService.CreateSlotCommand(request.serviceId(),request.serviceDate(),request.startTime(),request.endTime(),request.capacity())));}
    ...
    public ReservationResponse cancel(
            ...
            @PathVariable("reservationNo") String reservationNo){return ReservationResponse.from(service.cancel(scope(),reservationNo));}
