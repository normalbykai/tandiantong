...
public class ReservationResponse {
    ...
    @Schema(description = "预约取号，取消后可能为空", example = "A001") private final String pickupNo;

    private ReservationResponse(ReservationPersistenceService.ReservationResult result) {
        this.reservationNo = result.reservationNo(); this.status = result.status(); this.pickupNo = result.pickupNo();
    }
