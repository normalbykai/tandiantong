...
public class AdminCatalogController {
    ...
    public CreatedProductResponse create(@Valid @RequestBody CreateProductRequest request) {
        ...
        TenantStoreScope scope =
                new TenantStoreScope(
                        currentUser.tenantId(), currentUser.storeId(), currentUser.userId());
        return CreatedProductResponse.from(
                catalogPersistenceService.createProduct(
                        scope,
                        new CatalogPersistenceService.CreateCatalogProductCommand(
                                request.productName(),
                                request.categoryName(),
                                request.basePriceCent(),
                                request.onShelf(),
                                request.skus().stream()
                                        .map(
                                                sku ->
                                                        new CatalogPersistenceService
                                                                .CreateCatalogSkuCommand(
    ...
    public List<ProductResponse> list() {
        return catalogPersistenceService.listProducts(scope()).stream()
