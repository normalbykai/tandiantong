<template>
  <ListPageLayout title="平台账号" description="维护平台运营账号及所属角色；账号状态和敏感操作由服务端再次鉴权。" eyebrow="权限管理">
    ...
    <template #filters>
      ...
      <el-select v-model="statusFilter" class="page-filter-select" clearable placeholder="账号状态"><el-option label="已启用" value="ENABLED" /><el-option label="已停用" value="DISABLED" /></el-select>
    ...
    <AppDataTable :data="filteredAccounts" :loading="loading" row-key="id" empty-text="暂无平台账号" :total="filteredAccounts.length">
      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-if="hasPermission('platform:account:status:update')" link :type="row.status === 'ENABLED' ? 'danger' : 'primary'" @click="toggleStatus(row)">{{ row.status === 'ENABLED' ? '停用' : '启用' }}</el-button>
        </template>
      </el-table-column>
    </AppDataTable>
  </ListPageLayout>
...
const enabledCount = computed(() => accounts.value.filter(item => item.status === 'ENABLED').length)
