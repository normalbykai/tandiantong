...
function tenantStatusClass(value: TenantOverview['status']) { return value === 'ENABLED' ? 'status-enabled' : value === 'PENDING_ENABLE' ? 'status-pending' : 'status-disabled' }
function tagStyle(dictionaryType: string, itemCode: string) {
  const type = dictionary.dictTagType(dictionaryType, itemCode)
  const colors: Record<string, { color: string; borderColor: string; background: string }> = {
    success: { color: '#168b61', borderColor: '#b9e7d5', background: 'linear-gradient(135deg,#edfbf4,#e3f8ef)' },
    warning: { color: '#b87519', borderColor: '#f0d29f', background: 'linear-gradient(135deg,#fff8ea,#fff1d7)' },
    danger: { color: '#a23e3e', borderColor: '#efd4d4', background: 'linear-gradient(135deg,#fcf3f3,#f8e8e8)' },
    info: { color: '#596775', borderColor: '#dfe4e8', background: 'linear-gradient(135deg,#f7f9fa,#eef2f4)' }
  }
  return colors[type] ?? {}
}
