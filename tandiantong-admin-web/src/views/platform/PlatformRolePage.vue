<template>
  <PageHeader title="平台角色" description="配置平台职责角色和权限范围；预设角色和自定义角色均可按授权规则维护。" eyebrow="权限管理"><template #action><el-button type="primary" @click="openCreate">新增角色</el-button></template></PageHeader>
  <div class="info-banner">平台角色仅适用于平台管理端；商户后台使用独立的角色与权限域，两者不能混用。</div>
  <section class="content-card"><div class="table-summary"><div>共 <b>{{ filteredRoles.length }}</b> 条记录</div><div class="table-summary-filter"><el-input v-model="keyword" :prefix-icon="Search" clearable placeholder="搜索角色名称或标识" /><el-button :icon="RefreshCw" circle :loading="loading" aria-label="刷新列表" @click="load" /></div></div><el-table v-loading="loading" :data="filteredRoles" class="data-table" empty-text="暂无平台角色"><el-table-column label="角色名称" min-width="160"><template #default="{ row }"><b class="primary-cell">{{ row.name }}</b></template></el-table-column><el-table-column label="角色标识" min-width="230"><template #default="{ row }"><code>{{ row.roleCode }}</code></template></el-table-column><el-table-column prop="description" label="角色说明" min-width="260" show-overflow-tooltip /><el-table-column label="角色类型" width="120"><template #default="{ row }"><el-tag class="role-kind" :class="row.systemRole ? 'role-system' : 'role-custom'">{{ row.systemRole ? '系统预置' : '自定义' }}</el-tag></template></el-table-column><el-table-column label="状态" width="110"><template #default="{ row }"><el-tag class="status-tag" :class="row.status === 'ENABLED' ? 'status-enabled' : 'status-disabled'">{{ row.status === 'ENABLED' ? '启用' : '停用' }}</el-tag></template></el-table-column><el-table-column label="操作" width="250" fixed="right"><template #default="{ row }"><el-button link type="primary" @click="managePermissions(row)">配置权限</el-button><el-button link @click="openEdit(row)">编辑</el-button><el-button link :type="row.status === 'ENABLED' ? 'danger' : 'primary'" @click="toggleStatus(row)">{{ row.status === 'ENABLED' ? '停用' : '启用' }}</el-button></template></el-table-column></el-table></section>
